void TProfile_amtq_inner_trans_macro() {
        TTree* tree = (TTree*)gROOT->FindObject("mul");
        Long64_t nEntries = tree->GetEntries();
        tree->SetBranchStatus("*", false);

        std::cout << "Inner Transition" << endl;
	...
	for (int i = 0; i < 7; ++i) {
        ...
        for (int i=0; i<7; ++i) {
                l_detNames[i] = Form("asym_tq%02d_r5l", 4*i+4);
                r_detNames[i] = Form("asym_tq%02d_r5r", 4*i+2);
        ...
        TProfile* h = new TProfile("it", "Inner Transition Segments;Detector Number;Asymmetry", 16, -0.5, 15.5, ymin, ymax);
