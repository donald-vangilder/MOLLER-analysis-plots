void TProfile_ymtq_closed_ring6_macro() {
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("yield_tq%02d_r6", 4*i+1);
    ...
    TProfile* h = new TProfile("ring6c", "Ring 6 Closed Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
