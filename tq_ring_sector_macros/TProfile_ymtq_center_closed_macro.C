void TProfile_ymtq_center_closed_macro() {
        gDirectory->Delete("*");
	...
	Double_t fmin =  1e30, fmax = -1e30;
        ...
        for (int i=0; i<7; ++i) {
                detNames[i] = Form("yield_tq%02d_r5c", 4*i+1);
	...
	std::cout << "Histogram:      Min = " << ymin << ", Max = " << fmax << endl;
        ...
        TProfile* h = new TProfile("cc", "Center Closed Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
