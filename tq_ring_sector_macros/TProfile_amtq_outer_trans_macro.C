void TProfile_amtq_outer_trans_macro() {
        TTree* tree = (TTree*)gROOT->FindObject("mul");
        Long64_t nEntries = tree->GetEntries();
        tree->SetBranchStatus("*", false);

        std::cout << "Outer Transition" << endl;
	...
	for (int i = 0; i < 7; ++i) {
        ...
        for (int i=0; i<7; ++i) {
                l_detNames[i] = Form("asym_tq%02d_r5l", 4*i+2);
                r_detNames[i] = Form("asym_tq%02d_r5r", 4*i+4);
        ...
        TProfile* h = new TProfile("ot", "Outer Transition Segments;Detector Number;Asymmetry", 16, -0.5, 15.5, ymin, ymax);
