void TProfile_ymtq_ring5_macro() {
    gDirectory->Delete("*");
    ...
    std::cout << "\n\n\n\nAll Ring 5 Segments (Profile)" << endl;
    ...
    for (int i = 0; i < 28; ++i) {
        detNames_l[i] = Form("yield_tq%02d_r5l", i+1);
        detNames_c[i] = Form("yield_tq%02d_r5c", i+1);
        detNames_r[i] = Form("yield_tq%02d_r5r", i+1);
    ...
    TProfile* h = new TProfile("r5", "All Ring 5 Segment Yields; ;Yield", 88, -2.5, 85.5, ymin, ymax);
