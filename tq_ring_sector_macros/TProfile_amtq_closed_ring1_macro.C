void TProfile_amtq_closed_ring1_macro() {
    gDirectory->Delete("*");
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("asym_tq%02d_r1", 4*i+1);
    ...
    TProfile* h = new TProfile("ring1c", "Ring 1 Closed Segments;Detector Number;Asymmetry", 9, -0.5, 8.5, ymin, ymax);
