void TProfile_amtq_trans_ring2_macro() {
    TTree* tree = (TTree*)gROOT->FindObject("mul");
    Long64_t nEntries = tree->GetEntries();
    tree->SetBranchStatus("*", false);

    std::cout << "Transition Ring 2" << std::endl;
    ...
    for (int i = 0; i < 14; ++i) {
        detNames[i] = Form("asym_tq%02d_r2", 2*i+2);
    ...
    TProfile* h = new TProfile("ring2t", "Ring 2 Transition Segments;Detector Number;Asymmetry", 16, -0.5, 15.5, ymin, ymax);
