void TProfile_ymtq_closed_ring3_macro() {
    TTree* tree = (TTree*)gROOT->FindObject("mul");
    Long64_t nEntries = tree->GetEntries();
    tree->SetBranchStatus("*", false);

    std::cout << "Closed Ring 3" << endl;
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("yield_tq%02d_r3", 4*i+1);
    ...
    TProfile* h = new TProfile("ring3c", "Ring 3 Closed Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
