void TProfile_amtq_closed_ring2_macro() {
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("asym_tq%02d_r2", 4*i+1);
    ...
    TProfile* h = new TProfile("ring2c", "Ring 2 Closed Segments;Detector Number;Asymmetry", 9, -0.5, 8.5, ymin, ymax);
