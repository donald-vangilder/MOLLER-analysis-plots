void TProfile_amtq_edge_closed_macro() {
	...
	for (int i = 0; i < 7; ++i) {
        ...
        for (int i=0; i<7; ++i) {
                l_detNames[i] = Form("asym_tq%02d_r5l", 4*i+1);
                r_detNames[i] = Form("asym_tq%02d_r5r", 4*i+1);
        ...
        TProfile* h = new TProfile("ec", "Edge Closed Segments;Detector Number;Asymmetry", 16, -0.5, 15.5, ymin, ymax);
