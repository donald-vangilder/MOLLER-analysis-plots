void ymtq_all_ring3_macro() {
    TTree* tree = (TTree*)gROOT->FindObject("mul");
    Long64_t nEntries = tree->GetEntries();
    tree->SetBranchStatus("*", false);

    std::cout << "All Ring 3 Segments" << std::endl;
    ...
    for (int i = 0; i < 28; ++i) {
        detNames[i] = Form("yield_tq%02d_r3", i+1);
    ...
    TH2F* h = new TH2F("ring3a", "All Ring 3 Segments;Detector Number;Yield", 30, -0.5, 29.5, 1000, ymin, ymax);
