void TProfile_ymtq_closed_ring2_macro() {
    TTree* tree = (TTree*)gROOT->FindObject("mul");
    Long64_t nEntries = tree->GetEntries();
    tree->SetBranchStatus("*", false);

    std::cout << "Closed Ring 2" << endl;
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("yield_tq%02d_r2", 4*i+1);
    ...
    TProfile* h = new TProfile("ring2c", "Ring 2 Closed Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
