void TProfile_amtq_center_open_macro() {
        TTree* tree = (TTree*)gROOT->FindObject("mul");
        Long64_t nEntries = tree->GetEntries();
	tree->SetBranchStatus("*", false);

        std::cout << "Center Open" << endl;
	...
	Double_t fmin =  1e30, fmax = -1e30;
        ...
        for (int i=0; i<7; ++i) {
                detNames[i] = Form("asym_tq%02d_r5c", 4*i+3);
	...
	std::cout << "Histogram:     Min = " << ymin << ", Max = " << fmax << endl;
        ...
        TProfile* h = new TProfile("co", "Center Open Segments;Detector Number;Asymmetry", 9, -0.5, 8.5, ymin, ymax);
