void TProfile_ymtq_closed_ring1_macro() {
    gDirectory->Delete("*");
    ...
    std::cout << "\n\n\n\nStart of new data" << endl;
    std::cout << "Closed Ring 1" << endl;

    const char* detNames[7];
    TLeaf* leaves[7];
    Double_t vmin[7], vmax[7];
    Double_t fmin =  1e30, fmax = -1e30;

    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("yield_tq%02d_r1", 4*i+1);
    ...
    TProfile* h = new TProfile("ring1c", "Ring 1 Closed Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
