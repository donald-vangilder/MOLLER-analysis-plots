void TProfile_ymtq_center_open_macro() {
	...
	Double_t fmin =  1e30, fmax = -1e30;
        ...
        for (int i=0; i<7; ++i) {
                detNames[i] = Form("yield_tq%02d_r5c", 4*i+3);
	...
	std::cout << "Histogram:      Min = " << ymin << ", Max = " << fmax << endl;
        ...
        TProfile* h = new TProfile("co", "Center Open Segments;Detector Number;Yield", 9, -0.5, 8.5, ymin, ymax);
