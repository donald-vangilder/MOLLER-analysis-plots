void TProfile_amtq_open_ring1_macro() {
    TTree* tree = (TTree*)gROOT->FindObject("mul");
    Long64_t nEntries = tree->GetEntries();
    tree->SetBranchStatus("*", false);

    std::cout << "Open Ring 1" << endl;
    ...
    for (int i = 0; i < 7; ++i) {
        detNames[i] = Form("asym_tq%02d_r1", 4*i+3);
    ...
    TProfile* h = new TProfile("ring1o", "Ring 1 Open Segments;Detector Number;Asymmetry", 9, -0.5, 8.5, ymin, ymax);
