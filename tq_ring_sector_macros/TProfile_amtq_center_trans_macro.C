void TProfile_amtq_center_trans_macro() {
        TTree* tree = (TTree*)gROOT->FindObject("mul");
        Long64_t nEntries = tree->GetEntries();
	tree->SetBranchStatus("*", false);

        std::cout << "Center Transition" << endl;
	...
	Double_t fmin =  1e30, fmax = -1e30;
        ...
        for (int i=0; i<14; ++i) {
                detNames[i] = Form("asym_tq%02d_r5c", 2*i+2);
	...
	std::cout << "Histogram:     Min = " << ymin << ", Max = " << fmax << endl;
        ...
        TProfile* h = new TProfile("ct", "Center Transition Segments;Detector Number;Asymmetry", 16, -0.5, 15.5, ymin, ymax);
