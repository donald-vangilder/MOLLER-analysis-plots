void sm_mean_macro() {
	...
	for (int i=0; i<28; ++i) {
	        detNames[i] = Form("asym_sm%02d", i+1);
		...
		TString histname = Form("hsm%d", i+1);
        ...
        TH2F* h = new TH2F ("smm", "Shower Max Detector Mean Asymmetries;Detector Number;Mean Asymmetry", 30, -0.5, 29.5, 1000, ymin, ymax);
